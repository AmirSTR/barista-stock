...
async def confirm_invoice(
    ...
    query = (
        select(SupplyInvoice)
        .options(selectinload(SupplyInvoice.items))
        .where(SupplyInvoice.id == invoice_id)
    )
    result = await db.execute(query)
    invoice = result.scalar_one_or_none()
    ...
    if invoice.status == SupplyStatus.CONFIRMED:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invoice is already confirmed")

    # Add quantities to real_qty in stocks
    for item in invoice.items:
        if item.product_id:
            stock_res = await db.execute(select(Stock).where(Stock.product_id == item.product_id))
            stock = stock_res.scalar_one_or_none()
            if stock:
                stock.real_qty += item.quantity
            else:
                # Create stock if missing
                stock = Stock(product_id=item.product_id, real_qty=item.quantity, reserved_qty=0.0)
                db.add(stock)
