...
class OCRService:
    ...
    async def parse_invoice_photo(cls, image_bytes: bytes) -> InvoiceOCRResponse:
        ...
        if provider == "gemini" and gemini_key:
            model = settings.OCR_MODEL or "gemini-2.5-flash"
            raw_json_str = await cls._call_gemini_vision(image_bytes, gemini_key, model)
        elif provider == "openai" and openai_key:
            model = settings.OCR_MODEL or "gpt-4o-mini"
            raw_json_str = await cls._call_openai_vision(image_bytes, openai_key, model)
        elif gemini_key:
            # Auto-fallback to Gemini if gemini key is present
            raw_json_str = await cls._call_gemini_vision(
                image_bytes, gemini_key, settings.OCR_MODEL or "gemini-2.5-flash"
            )
        elif openai_key:
            # Auto-fallback to OpenAI if openai key is present
            raw_json_str = await cls._call_openai_vision(
                image_bytes, openai_key, settings.OCR_MODEL or "gpt-4o-mini"
            )
        else:
            logger.warning("No OCR API key configured (GEMINI_API_KEY / OPENAI_API_KEY). Returning demo/empty response.")
            # Graceful demo response when running in local development without API keys
