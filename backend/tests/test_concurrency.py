...
async def test_order_ship_and_cancel_api_endpoints(client: AsyncClient, db_session: AsyncSession):
    ...
    # A terminal order cannot be moved back to packing and shipped twice.
    ...
    second_ship_resp = await client.post(f"/api/orders/{order_id}/ship")
    assert second_ship_resp.status_code == 400
    stock_after_retries = await client.get("/api/v1/stocks/1")
    assert stock_after_retries.json()["real_qty"] == 40.0
